...
def arr2str(x, d=3):
    return (
        np.array2string(x, formatter={"float_kind": lambda x: f"%.{d}f" % x})
        .replace("\n", "")
        .replace(" ", ",")
    )
...
class parse_args:
    ...
    def add(self, arg, default, type, help=None):
        ...
        key = arg[0].replace("=", "")
        self.info.append(
            {"key": key, "type": type, "value": default, "arg": [f"--{key}"]}
        )
        if len(arg) == 2:
    ...
    def usage(self, err):
        for type, info in self.help:
            ...
            if type == "opt":
                ...
                help = helps[0]
                if len(arg) == 1:
                    print("--%-15s : %s" % (arg[0], help))
                if len(arg) == 2:
                    print(
                        "--%-10s -%-3s : %s" % (arg[0], arg[1].replace(":", ""), help)
                    )
                for help in helps[1:]:
                    print("%19s %s" % ("", help))
        print(f"< {err} >")
