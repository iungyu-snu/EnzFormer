...
def get_TrR_model(protocol="fixbb", L=None, num_models=1, hard=True, use_theta=True):
    ...
    def get_bkg_loss(x, eps=1e-8):
        loss = -tf.reduce_sum(
            x[1] * (tf.math.log(x[1] + eps) - tf.math.log(x[0] + eps)), -1
        )
        loss = tf.reduce_mean(loss) / 4
        return loss[None]
    ...
    def _fixbb_model(seq, true):
        ...
        return {"cce_loss": cce_loss[0], "grad": grad[0], "pred": pred[0]}

    def _hallucination_model(seq, bkg):
        bkg_loss, grad, pred = model.predict([seq[None], bkg[None]])
        return {"bkg_loss": bkg_loss[0], "grad": grad[0], "pred": pred[0]}

    def _partial_model(seq, true, bkg, pos_idx, pos_idx_ref=None):
        if pos_idx_ref is None:
            pos_idx_ref = pos_idx
        cce_loss, bkg_loss, grad, pred = model.predict(
            [seq[None], true[None], bkg[None], pos_idx[None], pos_idx_ref[None]]
        )
        return {
            "cce_loss": cce_loss[0],
            "bkg_loss": bkg_loss[0],
