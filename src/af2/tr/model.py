...
class mk_tr_model(design_model):
    def __init__(
        ...
        self._model_params = [
            get_model_params(f"{self._data_dir}/models/model_xa{k}.npy")
            for k in list("abcde")
        ]
        ...
        def _get_loss(outputs, opt):
            ...
            if self.protocol in ["hallucination", "partial"]:
                ...
                for k in ["dist", "omega", "theta", "phi"]:
                    aux["losses"]["bkg"][k] = (
                        -(p[k] * (log_p[k] - log_q[k])).sum(-1).mean()
                    )
            ...
            losses = {
                k: (tree_multi(v, w[k]) if k in w else v)
    ...
    def prep_inputs(
        ...
        **kwargs,
        ...
        if self.protocol in ["fixbb", "partial"]:
            ...
            self._feats = _np_get_6D_binned(
                self._batch["all_atom_positions"], self._batch["all_atom_mask"]
            )

            self._len = len(self._batch["aatype"])
            self.opt["weights"]["cce"] = {
                "dist": 1 / 6,
                "omega": 1 / 6,
                "theta": 2 / 6,
                "phi": 2 / 6,
            ...
            if atoms_to_exclude is not None:
                if "N" in atoms_to_exclude:
                    # theta = [N]-CA-CB-CB
                    self.opt["weights"]["cce"] = dict(
                        dist=1 / 4, omega=1 / 4, phi=1 / 2, theta=0
                    )
        ...
        if self.protocol in ["hallucination", "partial"]:
            ...
            for n in range(1, 6):
                model_params = get_model_params(
                    f"{self._data_dir}/bkgr_models/bkgr0{n}.npy"
                )
                self._bkg_feats.append(self._bkg_model(model_params, key, self._len))
            self._bkg_feats = jax.tree_map(
                lambda *x: jnp.stack(x).mean(0), *self._bkg_feats
            )

            # reweight the background
            self.opt["weights"]["bkg"] = dict(
                dist=1 / 6, omega=1 / 6, phi=2 / 6, theta=2 / 6
            )
    ...
    def run(self, model=None, backprop=True, average=True):
        ...
        for n in model_num:
            ...
            if backprop:
                (loss, aux), grad = self._grad_fn(
                    self.params, model_params, self.opt, self.key()
                )
            else:
    ...
    def step(self, model=None, backprop=True, callback=None, save_best=True, verbose=1):
    ...
    def plot(self, mode="preds", get_best=True, dpi=100):
        ...
        if mode == "preds":
            aux = (
                self.aux if (self._best_aux is None or not get_best) else self._best_aux
            )
            x = aux["outputs"]
        elif mode == "feats":
            x = self._feats
    ...
    def get_loss(self, k=None, get_best=True):
        ...
        if k is None:
            return {
                k: self.get_loss(k, get_best=get_best) for k in aux["losses"].keys()
            }
        losses = aux["losses"][k]
