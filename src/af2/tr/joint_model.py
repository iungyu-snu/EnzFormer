...
class mk_af_tr_model:
    def __init__(
        self, protocol="fixbb", use_templates=False, recycle_mode="last", num_recycles=0
    ):
        ...
        if protocol == "binder":
            ...
            def _prep_inputs(
                pdb_filename, chain, binder_len=50, binder_chain=None, **kwargs
            ):
                self.af.prep_inputs(
        ...
        if protocol == "fixbb":
            ...
            def _prep_inputs(pdb_filename, chain, pos=None, fix_seq=False, **kwargs):
                flags = dict(
                    pdb_filename=pdb_filename, chain=chain, pos=pos, fix_seq=fix_seq
                )
    ...
    def joint_design(self, iters=100, tr_weight=1.0, tr_seed=None, **kwargs):
        self.af.design(
            iters,
            callback=self.tr.af_callback(weight=tr_weight, seed=tr_seed),
            **kwargs
        )
