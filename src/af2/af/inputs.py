...
from af2.shared.model import soft_seq
from af2.af.alphafold.common import residue_constants
from af2.af.alphafold.model import model
from af2.af.alphafold.model.config import CONFIG
...
class _af_inputs:
    def _get_seq(self, params, opt, aux, key):
        ...
        if "pos" in opt and "fix_seq" in opt:
            ...
            if self.protocol == "partial":
                fix_seq = lambda x: jnp.where(
                    opt["fix_seq"], x.at[..., p, :].set(seq_ref), x
                )
            else:
                fix_seq = lambda x: jnp.where(
                    opt["fix_seq"], x.at[..., p, :].set(seq_ref[..., p, :]), x
                )
        ...
        if self.protocol in ["fixbb", "hallucination"] and self._copies > 1:
            seq = jax.tree_map(
                lambda x: expand_copies(x, self._copies, self._args["block_diag"]), seq
            )
        return seq
    ...
    def _update_template(self, inputs, opt, key):
        ...
        pb, pb_mask = model.modules.pseudo_beta_fn(
            aatype, self._batch["all_atom_positions"], self._batch["all_atom_mask"]
        )

        bounds = [0] + list(np.cumsum(self.lens))
        if self.use_dgram:
            template_dgram = model.modules.dgram_from_positions(
                pb,
                **self._runner.config.model.embeddings_and_evoformer.template.dgram_features
            )
            template_mask_2d = self._batch["mask_dgram"]
            template_dgram = template_dgram * template_mask_2d[..., None]
            dgram_mask = jax.random.bernoulli(
                key, 1 - opt["template"]["dropout"], (L, L)
            )
            template_dgram *= dgram_mask[..., None]
            inputs["template_dgram"] = jnp.stack(
                [template_dgram, jnp.zeros_like(template_dgram)]
            )
            inputs["template_mask_2d"] = jnp.stack(
                [template_mask_2d, jnp.zeros_like(template_mask_2d)]
            )
            inputs["template_aatype"] = jnp.stack([template_aatype, template_aatype])
            inputs["template_all_atom_positions"] = (
                inputs["template_all_atom_positions"]
                .at[0]
                .set(self._batch["all_atom_positions"])
            )
            inputs["template_all_atom_mask"] = (
                inputs["template_all_atom_mask"].at[0].set(self._batch["all_atom_mask"])
            )

            if self._args["rm_template_seq"]:
                inputs["template_all_atom_mask"] = (
                    inputs["template_all_atom_mask"].at[:, :, 5:].set(0)
                )
        else:
                ...
                if self.split_templates:
                    for ith, iclique in enumerate(self.cliques):
                        for ichain in iclique:
                            ...
                            inputs[k] = (
                                inputs[k]
                                .at[ith, bounds[ichain] : bounds[ichain + 1]]
                                .set(v_set)
                            )
                else:
            ...
            inputs["template_all_atom_mask"] = (
                inputs["template_all_atom_mask"].at[:, n:].multiply(pos_mask[n:, None])
            )
            inputs["template_pseudo_beta_mask"] = (
                inputs["template_pseudo_beta_mask"].at[:, n:].multiply(pos_mask[n:])
            )
    ...
    def _update_template_complex(self, inputs, atom_position, atom_mask):
        ...
        pb, pb_mask = model.modules.pseudo_beta_fn(
            self._wt_aatype, atom_position, atom_mask
        )
        if self.use_dgram:
            template_dgram = model.modules.dgram_from_positions(
                pb,
            ...
            inputs["template_dgram"] = (
                inputs["template_dgram"].at[-1].set(template_dgram)
            )
            inputs["template_mask_2d"] = (
                inputs["template_mask_2d"]
                .at[-1]
                .set(jnp.ones_like(inputs["template_mask_2d"][0]))
            )
        else:
            inputs["template_pseudo_beta"] = (
                inputs["template_pseudo_beta"].at[-1].set(pb)
            )
            inputs["template_pseudo_beta_mask"] = (
                inputs["template_pseudo_beta_mask"].at[-1].set(pb_mask)
            )
...
def update_seq(seq, inputs, seq_1hot=None, seq_pssm=None, msa_input=None):
    ...
    if msa_input is None:
        msa_feat = (
            jnp.zeros_like(inputs["msa_feat"])
            .at[0, :, :22]
            .set(seq_1hot[0])
            .at[0, :, 25:47]
            .set(seq_pssm[0])
    ...
    if seq.ndim == 3:
        target_feat = (
            jnp.zeros_like(inputs["target_feat"]).at[..., :20].set(seq[0, ..., :20])
        )
    else:
        target_feat = (
            jnp.zeros_like(inputs["target_feat"]).at[..., :20].set(seq[..., :20])
        )
...
def update_aatype(aatype, inputs):
    r = residue_constants
    a = {
        "atom14_atom_exists": r.restype_atom14_mask,
...
def crop_feat(feat, pos, model_runner, add_batch=True):
    ...
    if feat is None:
        ...
        for k, v in feat.items():
            ...
            elif k in ["mask_d", "mask_d_w", "mask_dgram"]:
                new_feat[k] = v[pos, :][:, pos]
            elif k in ["template_dgram", "template_mask_2d"]:
                new_feat[k] = v[:, pos][:, :, pos]
            elif k == "masks_rest":
                masks = {}
