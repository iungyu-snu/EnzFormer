...
import numpy as np
import joblib
from af2.shared.utils import copy_dict, Key, dict_to_str, to_float
...
class _af_design:
    ...
    def run(self, model=None, backprop=True, crop=True, average=True, callback=None):
        ...
        if crop and max_L < L:
            ...
            p = np.arange(L)
            masks = {"1v1": mask_1v1, "1vN": mask_1vN, "MvN": mask_MvN, "non": mask_non}
        ...
        for n, name in enumerate(self._model_names):
            if "openfold" in name:
                if self._args["use_openfold"]:
                    ns.append(n)
                    ns_name.append(name)
            elif self._args["use_alphafold"]:
                ns.append(n)
                ns_name.append(name)
        ...
        if average:
            ...
            self.aux["losses"] = jax.tree_map(
                lambda x: x.mean(0), outs["aux"]["losses"]
            )
    ...
    def _recycle(self, model_params, backprop=True):
        ...
        if mode in ["backprop", "add_prev"]:
            ...
            if mode == "average":
                ...
                self.set_opt(
                    recycles=jax.random.randint(self.key(), [], 0, recycles + 1)
                )
                out = self._single(model_params, backprop)
    ...
    def step(
        ...
        if grad_clip and tmp > 3:
            self.grad["seq"] = jnp.clip(
                self.grad["seq"], a_min=-clip_range, a_max=clip_range
            )
        ...
        if crop:
            ...
            if (flag_loss * flag_valid).sum() == flag_valid.sum():
                loss_sample = [
                    0 if iloss is None else iloss for iloss in self.loss_history
                ]
    ...
    def save_results(self, save_best=False, verbose=1):
        ...
        if "pae" in self.aux:
            traj["pae"] = np.asarray(self.aux["pae"])
    ...
    def design(
        ...
        for i in range(iters):
            self.set_opt(
                soft=(soft + (e_soft - soft) * ((i + 1) / iters)),
            ...
            if i % save_every_n_step == 0:
                self.gen_inputs.append(
                    jax.tree_util.tree_map(np.array, self.aux["inputs"])
                )
                if self._args["crop"] is False:
                    self.gen_outputs.append(
                        jax.tree_util.tree_map(np.array, self.aux["pdb"])
                    )
