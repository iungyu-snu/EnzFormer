...
from af2.af.alphafold.data import pipeline
from af2.af.alphafold.common import protein, residue_constants
from af2.af.alphafold.model import all_atom, model
from af2.af.alphafold.model.tf import shape_placeholders
from af2.af.inputs import update_aatype
from af2.af.utils import read_msa

from af2.shared.protein import _np_get_cb, pdb_to_string
from af2.shared.utils import copy_dict
from af2.shared.model import order_aa
...
class _af_prep:
    ...
    def _prep_dock(
        ...
        **kwargs,
        ...
        init_mask += noise_mask * (1.0 - init_mask)

        seq_init = np.eye(20)[self._wt_aatype] * init_mask[..., None]
        seq_init += (
            np.random.normal(size=(self._len, 20)) * 0.1 * (1.0 - init_mask[..., None])
        )
        ...
        if use_initial:
            ...
            for ind, iclique in enumerate(cliques):
                for ichain in iclique:
                    ...
                    ipdb_pos = (
                        mat_rot[ind]
                        .apply(ipdb_pos.reshape([-1, 3]))
                        .reshape([self.lens[ichain], 37, 3])
                    )
            ...
            for ind, iclique in enumerate(cliques):
                ...
                for ichain in iclique:
                    ...
                    for ipos in range(3):
                        if code_sub[ipos] == "1":
                            pdb_chain_sub.append(
                                pdb_chains[ichain][:, :, ipos]
                                - pdb_min[ind][ipos]
                                + 5.0
                            )
                        else:
        ...
        self._inputs = prep_input_features(
            L=self._len, N=num_msa, T=num_templates, eN=1
        )
        ...
        if self._args["use_msa"]:
            feature_msa = {
                **pipeline.make_sequence_features(
                    sequence=wt_seq, description="none", num_res=self._len
                ),
                **pipeline.make_msa_features(
                    msas=[input_msa], deletion_matrices=[input_dlm]
                ),
            }
...
def prep_pdb(pdb_filename, chain=None, for_alphafold=True):
    ...
    for chain in chains:
        protein_obj = protein.from_pdb_string(
            pdb_to_string(pdb_filename), chain_id=chain
        )
        batch = {
