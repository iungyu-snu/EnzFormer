...
from af2.shared.protein import _np_kabsch, _np_get_6D_loss
from af2.af.alphafold.model import model, folding
from af2.af.alphafold.common import confidence_jax
...
class _af_loss:
    def _loss_dock(self, inputs, outputs, opt, aux, batch=None):
        ...
        # PAE
        self._get_dock_pw_loss(
            inputs, outputs, opt, aux, interface=True, mask_d=batch["mask_d"]
        )

        # dgram loss
        aatype = inputs["aatype"]
        dgram_cce = get_dgram_loss(
            batch, outputs, copies=copies, aatype=aatype, return_cce=True
        )
        mask = batch["mask_d"]
    ...
    def _get_dock_pw_loss(
        self, inputs, outputs, opt, aux, interface=False, mask_d=None
    ):
        ...
        if not interface:
            aux["losses"].update(
                {
                    "con": get_con_loss(
                        dgram, dgram_bins, offset=offset, **opt["con"]
                    ).mean(),
                    "pae": pae.mean(),
                }
            )
            ...
            for k, v in zip(["pae", "con"], [pae, dgram]):
                if k == "con":
                    x = get_con_loss(
                        v, dgram_bins, offset=offset, mask_intra=mask_d, **opt["con"]
                    ).mean()
                    ix = get_con_loss(
                        v,
                        dgram_bins,
...
def get_ptm(outputs):
    ...
    ptm = confidence_jax.predicted_tm_score_jax(
        outputs["predicted_aligned_error"]["logits"],
...
####################
def get_dgram_loss(
    batch, outputs=None, copies=1, aatype=None, pred=None, return_cce=False
):
...
def get_fape_loss(batch, outputs, model_config, use_clamped_fape=False):
    ...
    loss = {"loss": 0.0}
    folding.backbone_loss(
        loss,
        sub_batch,
        outputs["structure_module"],
        model_config.model.heads.structure_module,
    )
    return loss["loss"]
