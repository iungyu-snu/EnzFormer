...
from af2.af.alphafold.model import data, config, model

from af2.shared.model import design_model
from af2.shared.utils import Key

from af2.af.prep import _af_prep
from af2.af.loss import _af_loss, get_plddt, get_pae, get_ptm
from af2.af.utils import _af_utils, save_pdb_from_aux, cal_iptm
from af2.af.design import _af_design
from af2.af.inputs import _af_inputs, update_seq, crop_feat
...
class mk_af_model(design_model, _af_inputs, _af_loss, _af_prep, _af_design, _af_utils):
    def __init__(
        ...
        # initialize runner
        self._runner = model.RunModel(
            cfg, recycle_mode=recycle_mode, use_multimer=use_multimer
        )
    ...
    def gen_infer(self, root_path):
        ...
        for i in tqdm(range(steps)):
            ...
            i_outputs = self._runner.apply(
                self._model_params[0], self.key(), self._inputs
            )
            i_aux = {
                "atom_positions": i_outputs["structure_module"]["final_atom_positions"],
                "atom_mask": i_outputs["structure_module"]["final_atom_mask"],
                "residue_index": self.residue_index,
                "aatype": self._inputs["aatype"],
                "plddt": get_plddt(i_outputs),
                "pae": get_pae(i_outputs),
                "ptm": get_ptm(i_outputs),
            }
            ...
            self.gen_pdb.append(
                {
                    "atom_positions": i_aux["atom_positions"],
    ...
    def dis_infer(self, root_path):
        ...
        for i in tqdm(range(steps)):
            ...
            aatype_ = (
                aatype
                if prob_seq < 0.5
                else jnp.asarray(self.gen_inputs[i]["seq"]["pseudo"])
            )
            if self.protocol == "dock" and self._args["use_msa"]:
            ...
            idis_outputs = self._runner.apply(
                self._model_params[0], self.key(), self._inputs
            )
            i_aux = {
                "atom_positions": idis_outputs["structure_module"][
                    "final_atom_positions"
                ],
                "atom_mask": idis_outputs["structure_module"]["final_atom_mask"],
                "residue_index": self.residue_index,
                "aatype": self._inputs["aatype"],
                "plddt": get_plddt(idis_outputs),
                "pae": get_pae(idis_outputs),
                "ptm": get_ptm(idis_outputs),
            }
            i_aux = jax.tree_util.tree_map(np.asarray, i_aux)
            save_path = root_path + f"_{i+1}.pdb"
            save_pdb_from_aux(i_aux, save_path)
    ...
    def _get_model(self, callback=None):
        ...
        def _model(params, model_params, inputs, batch, key, opt):
            ...
            aux.update(
                {"inputs": {"seq": {"pseudo": seq["pseudo"], "pssm": seq["pssm"]}}}
            )
            # crop inputs
            if opt["crop_pos"].shape[0] < L:
                inputs = crop_feat(
                    inputs, opt["crop_pos"], self._runner, add_batch=False
                )
            ...
            aux["losses"] = {}
            self._get_loss(
                inputs=inputs, outputs=outputs, opt=opt, aux=aux, batch=batch
            )
