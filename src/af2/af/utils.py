...
from af2.shared.utils import update_dict
from af2.shared.plot import make_animation, show_pdb
from af2.shared.protein import renum_pdb_str
from af2.af.alphafold.common import protein
from af2.af.alphafold.common import confidence
from af2.af.alphafold.data import parsers
...
class _af_utils:
    ...
    def set_opt(self, *args, **kwargs):
        ...
        """
        for k in ["best_metric", "crop", "crop_len", "use_openfold", "use_alphafold"]:
            if k in kwargs:
        ...
        if "recycle_mode" in kwargs:
            if kwargs["recycle_mode"] in ["sample", "last"] and self._args[
                "recycle_mode"
            ] in ["sample", "last"]:
                self._args["recycle_mode"] = kwargs.pop("recycle_mode")
            else:
                print(
                    f"ERROR: use {self.__class__.__name__}(recycle_mode=...) to set the recycle_mode"
                )
    ...
    def save_pdb(self, filename=None, get_best=True, renum_pdb=True):
        ...
        p = {
            k: aux[k]
            for k in ["aatype", "residue_index", "atom_positions", "atom_mask"]
        }
    ...
    def plot_traj(self, dpi=100):
        ...
        if self.protocol in ["fixbb", "partial"] or (
            ...
            ax1.set_ylabel("RMSD", color="black")
            ax1_.set_ylabel("seqid", color="green")
...
def cal_iptm(ptm, asym_id):
    iptm = confidence.predicted_interface_tm_score(
        logits=ptm["logits"], breaks=ptm["breaks"], asym_id=asym_id, interface=True
    )
    return iptm


def read_msa(msas):
    # msas = abspath of a3m file (after alignment)
    with open(msas, "r+") as f:
        msa_content = f.read()
    input_msa, input_dlm = parsers.parse_a3m(msa_content)
