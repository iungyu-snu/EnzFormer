        ...
        for k, v in x.items():
            if v is not None:
                if k in d:
                    ...
                    raise Exception((f"ERROR: '{k}' not found in {list(d.keys())}"))
...
def to_float(x):
    ...
    if isinstance(x, dict):
        x = {k: to_float(y) for k, y in x.items()}
    elif hasattr(x, "__iter__"):
        x = [to_float(y) for y in x]
    else:
        x = float(x)
    return x


def dict_to_str(x, filt=None, keys=None, ok=None, print_str="", f=3):
