# import matplotlib
import numpy as np
from af2.shared.protein import _np_kabsch, alphabet_list
    ...
    **kwargs,
    ...
    if xyz is not None:
        ...
        if align_xyz:
            ...
            m = pos_mean.mean(0)
            rot_mtx = _np_kabsch(
                pos_mean - m, pos_mean - m, return_v=True, use_jax=False
            )
            pos = (pos - m) @ rot_mtx
    ...
    if xyz is not None:
        x_min, y_min, z_min = (
            np.minimum(np.mean([x.min(0) for x in pos], 0), pos_ref_full.min(0)) - 5
        )
        x_max, y_max, z_max = (
            np.maximum(np.mean([x.max(0) for x in pos], 0), pos_ref_full.max(0)) + 5
        )
    ...
    for k in range(len(seq)):
        ...
        if xyz is not None:
            if plddt is None:
                ims[-1].append(
                    plot_pseudo_3D(
                        pos[k], ax=ax1, line_w=line_w, zmin=z_min, zmax=z_max
                    )
                )
            else:
                ims[-1].append(
        ...
        if seq[k].shape[0] == 1:
            ims[-1].append(
                ax2.imshow(seq[k][0].T, animated=True, cmap="bwr_r", vmin=-1, vmax=1)
            )
        else:
