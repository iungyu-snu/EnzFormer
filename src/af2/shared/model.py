...
from af2.shared.utils import copy_dict, update_dict
from af2.shared.prep import rewire
...
class design_model:
    ...
    def set_seq(self, seq=None, mode=None, bias=None, rm_aa=None, **kwargs):
        ...
        if seq is None:
            ...
            if kwargs.pop("add_seq", False):
                print(
                    "WARNING: option 'add_seq' will soon be deprecated. To fix the sequence use:"
                )
    ...
    def rewire(self, order=None, offset=0, loops=0):
        self.opt["pos"] = rewire(
            length=self._pos_info["length"], order=order, offset=offset, loops=loops
        )
    ...
    def predict(self, seq=None, model=0):
        ...
        self.run(model=model, backprop=False, average=False, crop=False)
        return self.aux
...
def soft_seq(x, opt, key=None):
    ...
    if x.ndim == 3 and x.shape[0] > 1 and key is not None:
        ...
        seq["input"] = (
            seq["input"].at[0].set(seq["input"][n]).at[n].set(seq["input"][0])
        )
