...
def _np_get_6D(
    all_atom_positions, all_atom_mask=None, use_jax=True, for_trrosetta=False
):
    ...
    if for_trrosetta:
        out.update(
            {
                "dist": _np_len(b0, b1, use_jax=use_jax),
...
def jnp_rmsd(true, pred, add_dist=False):
    ...
    if add_dist:
        rmsd = (rmsd + _np_rmsdist(true, pred)) / 2
    return rmsd
...
def _np_get_6D_loss(
    ...
    f = {
        "T": _np_get_6D(true, mask, use_jax=use_jax),
...
def _np_get_6D_binned(all_atom_positions, all_atom_mask, use_jax=None):
    ...
    ref = _np_get_6D(
        all_atom_positions, all_atom_mask, use_jax=False, for_trrosetta=True
    )
    ...
    return {
        "dist": mtx2bins(ref["dist"], 2.0, 20.0, 37, mask=mask),
